...
function renderFavorites(page = 1) {
  ...
  const notFound = document.getElementById('my-favorites-notfound');
  ...
  list.innerHTML = currentPageItems
    .map(
      ex => `
   <li class="my-exercise-card">
  <div class="my-exercise-card-header">
    <div class="my-exercise-card-workout-box">
      <p class="my-exercise-card-workout-text">WORKOUT</p>

      <button class="my-exercise-card-workout-button" workout-id="${ex._id}">
    ...
    <button class="my-exercise-card-workout-start" workout-id="${ex._id}">
      <span>Start</span>
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
  ...
  <div class="my-exercise-card-title">
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      />
      <path
        d="M15.8448 7.30102C16.7564 7.30102 17.4954 6.56206 17.4954 5.65051C17.4954 4.73896 16.7564 4 15.8448 4C14.9333 4 14.1943 4.73896 14.1943 5.65051C14.1943 6.56206 14.9333 7.30102 15.8448 7.30102Z"
        fill="#F4F4F4"
      />
    </svg>
    <h3>${ex.name}</h3>
  ...
  <div class="my-exercise-card-info">
    <p class="my-exercise-card-info-p1"><span>Burned calories:</span> ${capitalizeFirstLetter(ex.burnedCalories + ' / 3 min')}</p>
    <p class="my-exercise-card-info-p2"><span>Body part:</span> ${capitalizeFirstLetter(ex.bodyPart)}</p>
    <p class="my-exercise-card-info-p3"><span>Target:</span>${capitalizeFirstLetter(ex.target)}</p>
  </div>
</li>`,
    )
    .join('');
  ...
  const buttonsStart = document.querySelectorAll('.my-exercise-card-workout-start');
  ...
  const buttonsDelete = document.querySelectorAll('.my-exercise-card-workout-button');
  ...
  if (totalPages > 3) {
    ...
    <div class="my-pagination-group">
      <button class="my-nav-btn" data-action="first" ${page === 1 ? 'disabled' : ''}>
      ...
      <button class="my-nav-btn" data-action="prev" ${page === 1 ? 'disabled' : ''}>
    ...
    <span class="my-page-numbers"></span>

    <div class="my-pagination-group">
      <button class="my-nav-btn" data-action="next" ${page === totalPages ? 'disabled' : ''}>
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <button class="my-nav-btn" data-action="last" ${page === totalPages ? 'disabled' : ''}>
        <svg width="31" height="24" viewBox="0 0 31 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const pageNumbersEl = pagination.querySelector('.my-page-numbers');
  if (totalPages > 3) {
    for (let i = Math.max(page - 1, 1); i <= Math.min(totalPages, Math.max(page - 1, 1) + 2); i++) {
      const btn = document.createElement('button');
      btn.className = 'my-page-btn';
      btn.textContent = i;
